...
const Assign = () => {
    ...
    const update = (i) => {
        ...
        formdata.append("assigned_to", userRef.current.value);
        ...
        <div>
            <div className="alltasks" style={!displayList ? { display: "none" } : { display: "block" }}>
                {sortedTask.map((task, id) => <div className="task" key={task.id}>
                    ...
                    <button className="edit" onClick={() => edit(task.message, task.priority, task.assigned_name, task.due_date, task.id)}>Add User</button></div>)}
            </div>
            <form className="create_form" style={displayList ? { display: "none" } : { display: "block" }} onSubmit={update}>
                <div className="input">
                    ...
                    <select ref={userRef}>
                        {
                            allUser.map((user, id) => <option key={user.id} value={user.id}>
                ...
                <button type="submit" className="create">Assign</button>
